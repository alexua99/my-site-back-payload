import { MigrateUpArgs, MigrateDownArgs, sql } from '@payloadcms/db-postgres'

export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ALTER TABLE "pages_blocks_section_block_icon_text_items"
      ALTER COLUMN "text" TYPE jsonb USING CASE
        WHEN "text" IS NULL OR "text" = '' THEN NULL
        ELSE jsonb_build_object('root', jsonb_build_object(
          'type', 'root',
          'children', jsonb_build_array(jsonb_build_object('type', 'text', 'text', "text", 'version', 1)),
          'direction', 'ltr',
          'format', '',
          'indent', 0,
          'version', 1
        ))
      END;

    ALTER TABLE "_pages_v_blocks_section_block_icon_text_items"
      ALTER COLUMN "text" TYPE jsonb USING CASE
        WHEN "text" IS NULL OR "text" = '' THEN NULL
        ELSE jsonb_build_object('root', jsonb_build_object(
          'type', 'root',
          'children', jsonb_build_array(jsonb_build_object('type', 'text', 'text', "text", 'version', 1)),
          'direction', 'ltr',
          'format', '',
          'indent', 0,
          'version', 1
        ))
      END;
  `)
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    ALTER TABLE "pages_blocks_section_block_icon_text_items"
      ALTER COLUMN "text" TYPE varchar USING ("text" #>> '{root,children,0,text}');
    ALTER TABLE "_pages_v_blocks_section_block_icon_text_items"
      ALTER COLUMN "text" TYPE varchar USING ("text" #>> '{root,children,0,text}');
  `)
}
